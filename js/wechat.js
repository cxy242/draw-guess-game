  ...
  ;(async () => {
    ...
    if (_needGen && window.callAI) {
      ...
      try {
        ...
        '8. mem_schedule_past：从聊天记录中提取过去3天发生的所有事件。每个事件必须有完整日期和时间段。格式：[{date:\'2026年9月12日\',events:[{time:\'下午3点\',event:\'和用户去咖啡厅\'}]}]。【强制】必须从聊天记录中提取事件，不能返回空数组。如果聊天中有任何活动、对话、计划、情绪变化，都必须记录。\n' +
        '9. mem_schedule_today：从聊天记录中提取今天的活动和安排，每项必须有时间段。格式：[{time:\'下午3点\',event:\'和用户聊天\'}]。【强制】聊天中提到的任何事情都要记录，不能返回空数组。\n' +
        '10. mem_schedule_tomorrow：从聊天记录中提取明天的计划，每项必须有时间段。格式同上。如果聊天中提到明天要做的事，必须记录。实在没有才返回空数组。\n' +
        '11. mem_agreements：与用户之间的约定或承诺。必须写明：约定内容+时间+当前状态。格式：[{event:\'周末一起吃饭\',time:\'周六晚上\',status:\'已约定\'}]。如果没有约定，返回空数组 []。\n\n' +
        '关键规则：\n' +
        '- 每个字段都不能为空字符串（数组可以为空[]）\n' +
        '- 时间格式必须为"YYYY年M月D日下午X点"（带年份+时间段），不能只写"3点"\n' +
