...
window.addToBatchMemory = function(source, charId, entry) {
  ...
  console.log('[BatchMemory] ' + source + ' 缓冲 +' + entry.title + ' (共' + buffer.length + '条)');
}
...
window.checkAndFlushBatchMemory = async function(source, charId) {
  ...
  console.log('[BatchMemory] ' + source + ' 达到10条，开始批量总结...');
  ...
  var prompt = '你是一个长期记忆整理器。请将以下' + buffer.length + '条' + sourceLabel + '互动记录，合并为1-3条精炼的长期记忆。\n\n' +
    ...
    '1. 使用第三人称叙述，客观平实\n' +
    '2. 合并相似事件，保留时间线\n' +
    '3. 每条记忆包含时间范围（如"X月X日-X月X日"）\n' +
    '4. 标题简短，内容150字以内\n' +
    '5. 禁止夸张情绪词汇和价值升华\n\n' +
    '互动记录：\n' + lines + '\n\n' +
    '返回JSON格式：\n' +
    '{"memories":[{"title":"简短标题","content":"第三人称客观记忆内容(含时间)","keywords":["关键词1","关键词2"],"valence":0,"arousal":0.3,"importance":5}]}';
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.memories) && parsed.memories.length > 0) {
      ...
      if (rows.length) {
        ...
        console.log('[BatchMemory] ' + source + ' 批量总结完成：' + rows.length + '条记忆');
      }
      // 清空缓冲
      try { localStorage.removeItem(key); } catch(_) {}
    }
  } catch(e) {
    console.warn('[BatchMemory] ' + source + ' 批量总结失败:', e);
    // 失败不清空，下次重试
  }
}
...
async function showXGenPostsDialog(user, genBtn, page) {
      ...
      '<div class="x-gen-section">' +
        ...
        '<div class="x-gen-chars" id="x-gen-chars">' +
          chars.map(function(c) {
            return '<label class="x-gen-char-pill"><input type="checkbox" data-id="' + c.id + '" checked><span>' + (c.nick || c.name) + '</span></label>'
          }).join('') +
  ...
  dialog.querySelector('.x-gen-close').addEventListener('click', function() {
    dialog.classList.add('closing')
    setTimeout(function(){ dialog.remove() }, 200)
  })
  dialog.querySelector('.x-gen-overlay').addEventListener('click', function() {
    dialog.classList.add('closing')
    setTimeout(function(){ dialog.remove() }, 200)
  })

  dialog.querySelector('.x-gen-confirm').addEventListener('click', async function() {
...
async function generatePostsForChar(char, preference) {
  ...
  var charDesc = char.name + '（' + (char.description || char.identity?.bio || char.signature || '普通用户').slice(0, 50) + '）'
  ...
  var prompt = '你是社交媒体内容生成器。请为角色 ' + char.name + ' 生成1条帖子，带完整评论互动。\n\n' +
    ...
    '可用NPC人设：\n' + npcSample + '\n\n' +
    '要求：\n' +
    '1. 帖子30-80字，真实自然，体现角色性格\n' +
    '2. 生成5条一级评论，来自不同NPC人设\n' +
    '3. 发帖人选择性回复2-3条\n' +
    '4. 被回复的NPC可以再回复发帖人\n' +
    '5. NPC之间可以互评，形成3层对话链\n\n' +
    '返回JSON：\n' +
    '{"posts":[{"content":"帖子内容","tags":["标签"],"comments":[{"name":"NPC名","content":"评论","replyToIndex":-1},{"name":"' + char.name + '","content":"回复","replyToIndex":0,"isAuthorReply":true}]}]}'
