...
fastify.get('/health', async () => ({
  ...
  currentRound: currentRound ? {
    answer: currentRound.answer || '(用户画)',
    artist: currentRound.artist,
    guesses: currentRound.guesses.length,
    elapsed: Math.floor((Date.now() - currentRound.startTime) / 1000),
  } : null,
  onlinePlayers: Object.keys(onlinePlayers).length,
  totalDrawings: savedDrawings.length,
  totalScores: Object.keys(scores).length,
  wordBank: WORD_BANK.length,
}));
...
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{background:#1a1a2e;color:#eee;font-family:system-ui,sans-serif;min-height:100vh;padding:20px}
h1{text-align:center;color:#87CEEB;margin-bottom:20px}
.card{background:#16213e;border-radius:12px;padding:16px;margin-bottom:12px;border:1px solid #2a2a4a}
.stat{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #2a2a4a}
.stat:last-child{border:none}
.label{color:#999}.val{color:#FFB6C1;font-weight:600}
.btn{padding:8px 16px;border-radius:8px;border:none;cursor:pointer;font-size:.9em;margin:4px}
.btn-danger{background:#e53935;color:#fff}
.btn-blue{background:#87CEEB;color:#1a1a2e}
...
.msg.ok{background:rgba(80,200,120,.18);color:#50C878}
...
<div class="card">
  <h3 style="color:#FFB6C1;margin-bottom:10px">操作</h3>
  <button class="btn btn-blue" onclick="refresh()">🔄 刷新状态</button>
  <button class="btn btn-danger" onclick="resetScores()">🗑 清空分数</button>
  <button class="btn btn-danger" onclick="clearGallery()">🖼 清空画廊</button>
  <button class="btn btn-danger" onclick="kickAll()">👢 踢出所有玩家</button>
</div>
<div class="card">
  <h3 style="color:#FFB6C1;margin-bottom:10px">最近画作</h3>
  <div id="gallery" class="log">加载中...</div>
</div>
<script>
const $=id=>document.getElementById(id);
function showMsg(text,ok){const m=$('msg');m.textContent=text;m.className='msg '+(ok?'ok':'err');m.style.display='block';setTimeout(()=>m.style.display='none',3000);}
async function refresh(){
  try{
    const r=await fetch('/health');const d=await r.json();
    const uptime=Math.floor(d.uptime/3600)+'h '+Math.floor(d.uptime%3600/60)+'m '+Math.floor(d.uptime%60)+'s';
    let roundHtml='<span style="color:#666">无</span>';
    if(d.currentRound){
      roundHtml='<span style="color:#50C878">'+d.currentRound.artist+' | '+d.currentRound.answer+' | '+d.currentRound.guesses+'次猜测 | '+d.currentRound.elapsed+'s</span>';
    }
    $('stats').innerHTML=[
      ['运行时间',uptime],
      ['当前回合',roundHtml],
      ['在线玩家',d.onlinePlayers+'人'],
      ['画廊作品',d.totalDrawings+'幅'],
      ['有分数的玩家',d.totalScores+'人'],
      ['词库大小',d.wordBank+'个'],
    ].map(([l,v])=>'<div class="stat"><span class="label">'+l+'</span><span class="val">'+v+'</span></div>').join('');
  }catch(e){showMsg('获取状态失败: '+e.message,false);}
}
async function loadGallery(){
  try{
    const r=await fetch('/api/gallery');const d=await r.json();
    if(!d.drawings||!d.drawings.length){$('gallery').textContent='画廊为空';return;}
    $('gallery').innerHTML=d.drawings.slice(-20).reverse().map(d=>
      (d.created_at||'').slice(0,16)+' | '+(d.artist==='AI'?'🤖 AI':'✏️ '+(d.author||'匿名'))+' | '+(d.answer||'无答案')+' | '+(d.guesses?d.guesses.length+'次猜测':'')
    ).join('\\n');
  }catch(e){$('gallery').textContent='加载失败';}
}
async function resetScores(){if(!confirm('确定清空所有分数？'))return;try{const r=await fetch('/api/admin/reset_scores',{method:'POST'});const d=await r.json();showMsg(d.message,true);refresh();}catch(e){showMsg('失败',false);}}
async function clearGallery(){if(!confirm('确定清空画廊？'))return;try{const r=await fetch('/api/admin/clear_gallery',{method:'POST'});const d=await r.json();showMsg(d.message,true);loadGallery();}catch(e){showMsg('失败',false);}}
async function kickAll(){if(!confirm('确定踢出所有玩家？'))return;try{const r=await fetch('/api/admin/kick_all',{method:'POST'});const d=await r.json();showMsg(d.message,true);refresh();}catch(e){showMsg('失败',false);}}
refresh();loadGallery();setInterval(refresh,10000);
</script>
</body>
</html>`;
...
fastify.post('/api/admin/reset_scores', async () => {
  scores = {};
  return { ok: true, message: '分数已清空' };
});

fastify.post('/api/admin/clear_gallery', async () => {
  savedDrawings = [];
  return { ok: true, message: '画廊已清空' };
});

fastify.post('/api/admin/kick_all', async () => {
  onlinePlayers = {};
  return { ok: true, message: '已踢出所有玩家' };
});
