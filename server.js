...
const GAME_URL = process.env.GAME_URL || 'https://web-production-54961.up.railway.app';
...
fastify.route({
  ...
  handler: async (req, reply) => {
    ...
    function api(method, path, body) {
      ...
      return fetch(`http://127.0.0.1:${process.env.PORT || 3001}${path}`, opts).then(r => r.json()).catch(e => ({ ok: false, message: e.message }));
    }

    const server = new McpServer({ name: 'draw-game', version: '3.0.0' });

    server.tool('draw_user_start', '用户要画画给你猜。告诉用户去画板画画。', {}, async () => {
      return { content: [{ type: 'text', text: `好的！你去这个链接画画吧：\n${GAME_URL}\n\n切换到「🖌️ 画板」标签，画完输入答案和署名，点「提交画作」，然后回来说"画好了"~` }] };
    });

    server.tool('draw_check', '用户说画好了时调用。查看用户的画作并猜是什么。', {}, async () => {
      ...
      return { content: [{ type: 'text', text: `画师: ${author}\n\n以下是这幅画的文字描述：\n${desc}\n\n画布范围：宽1000 x 高700，中心在(500,350)。\n请根据线条描述判断画的是什么。用 draw_submit_guess 提交猜测。` }] };
    });

    server.tool('draw_submit_guess', '提交猜测。', { guess: z.string().describe('你的猜测') }, async ({ guess }) => {
    ...
    server.tool('draw_ai_draw', '用户让你画画时调用。', {
      answer: z.string().describe('正确答案'),
      aliases: z.array(z.string()).optional().describe('其他可接受答案'),
      content: z.array(z.object({ tool: z.literal('polyline'), points: z.array(z.array(z.number())), color: z.string().optional().default('#4f454b'), width: z.number().optional().default(8) })).describe('线条数组'),
      ...
      return { content: [{ type: 'text', text: `画好了！以下是画作描述，发给用户猜：\n\n${desc}\n\n猜对了开心确认，猜错了鼓励再猜。` }] };
    });

    server.tool('draw_guess', '用户在聊天里猜AI画的画。', { guesser: z.string(), content: z.string() }, async ({ guesser, content }) => {
    ...
    server.tool('draw_score', '查看当前分数。', {}, async () => {
